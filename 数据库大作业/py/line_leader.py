...
def frame():  # 管理员界面
    ...
    root.title('公交管理系统')
    # 上
    lable0 = tk.Label(root, text='路队长登录', bg='pink', font=('微软雅黑', 50)).pack()
    # 下
    ...
    tk.Button(root, text='退出', font=('微软雅黑', 15), width=10, height=2, command=exit_line_leader).place(x=550, y=500)
    root.mainloop()
...
def login():  # 登录小窗口
    ...
    # 当我们输入账号和密码，点击确定时候，会调用ID模块里的id_check()函数，1是参数，表示其身份是路队长
